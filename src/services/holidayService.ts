...
export const backupCurrentHolidays = async (): Promise<boolean> => {
  try {
    ...
    for (const holiday of holidays) {
      // First check if this holiday date already exists in backup
      ...
      if (checkError) {
        console.error(`Error checking if holiday ${holiday.date} exists in backup:`, checkError);
        continue; // Skip this holiday but continue with others
      }
