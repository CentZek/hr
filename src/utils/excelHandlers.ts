...
export const exportApprovedHoursToExcel = (data: any) => {
  ...
  data.summary.forEach((employee: any) => {
    ...
    const workingDays = employee.working_week_dates?.filter((d: string) => (employee.hours_by_date?.[d] || 0) > 0).length || 0;
    const offDays = employee.working_week_dates?.filter((d: string) => (employee.hours_by_date?.[d] || 0) === 0).length || 0;
