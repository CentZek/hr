...
export const exportApprovedHoursToExcel = (data: any) => {
  ...
  data.summary.forEach((employee: any) => {
    ...
    const workingDays = employee.working_week_dates.filter((d: string) => (employee.hours_by_date[d]||0)>0).length;
    const offDays = employee.working_week_dates.filter((d: string) => (employee.hours_by_date[d]||0)===0).length;
  ...
  summaryData.push([
    ...
    totalWorkingDays,
    totalOffDays,
    (totalRegularHours / (totalEmployees || 1)).toFixed(2)
  ]);
  ...
  statsData.push(['Total Double-Time Hours', totalDoubleTimeHours.toFixed(2)]);
  statsData.push(['Total Payable Hours', totalPayableHours.toFixed(2)]);
  statsData.push(['Working Days', summaryData.slice(1).reduce((sum, row)=>sum + (parseInt(row[6])||0), 0)]);
  statsData.push(['Off-Days', summaryData.slice(1).reduce((sum, row)=>sum + (parseInt(row[7])||0), 0)]);
