...
export const exportToExcel = (employeeRecords: EmployeeRecord[]) => {
  ...
  employeeRecords.forEach(employee => {
    employee.days.forEach(day => {
      ...
      const checkIn = day.firstCheckIn ? format(day.firstCheckIn, 'MM/dd/yyyy HH:mm:ss') : 'Missing';
      const checkOut = day.lastCheckOut ? format(day.lastCheckOut, 'MM/dd/yyyy HH:mm:ss') : 'Missing';
      
      // Format date
      const date = day.date;
      
      // Get status
      const status = day.approved ? 'Approved' : 'Pending';
  ...
  const filename = `employee_records_${format(new Date(), 'yyyy-MM-dd_HH-mm-ss')}.xlsx`;
