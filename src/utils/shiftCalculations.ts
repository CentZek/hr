...
import { formatTime24H, ensureValidDate, safeGetHours, safeGetMinutes } from './dateTimeHelper';
...
export const determineShiftType = (
  ...
  if (!checkInTime || !(checkInTime instanceof Date) || isNaN(checkInTime.getTime())) {
    console.warn('Invalid check-in time provided to determineShiftType:', checkInTime);
    return 'morning'; // Default to morning shift if invalid
  }
...
export const isLateCheckIn = (checkIn: Date, shiftType: 'morning' | 'evening' | 'night' | 'canteen' | 'custom' | null): boolean => {
  if (!checkIn || !shiftType) return false;
  
  // Validate the date object
  if (!(checkIn instanceof Date) || isNaN(checkIn.getTime())) {
    console.warn('Invalid check-in time in isLateCheckIn:', checkIn);
    return false;
  }
  ...
  try {
    ...
    console.error('Error checking if check-in is late:', error);
  }
...
export const isEarlyLeave = (checkOut: Date, shiftType: 'morning' | 'evening' | 'night' | 'canteen' | 'custom' | null): boolean => {
  if (!checkOut || !shiftType) return false;
  
  // Validate the date object
  if (!(checkOut instanceof Date) || isNaN(checkOut.getTime())) {
    console.warn('Invalid check-out time in isEarlyLeave:', checkOut);
    return false;
  }
  ...
  try {
    ...
    if (!SHIFT_TIMES[shiftType]) {
      console.warn('Unknown shift type in isEarlyLeave:', shiftType);
      return false;
    }
    ...
    console.error('Error checking if check-out is early leave:', error);
  }
...
export const calculateHoursWorked = (checkInTime: Date, checkOutTime: Date): number => {
  ...
  if (!checkInTime || !checkOutTime || 
      ...
      isNaN(checkInTime.getTime()) || isNaN(checkOutTime.getTime())) {
    console.warn('Invalid input to calculateHoursWorked:', { checkInTime, checkOutTime });
    return 0;
  }
...
export const calculatePayableHours = (
  ...
  const validCheckIn = ensureValidDate(checkInTime);
  const validCheckOut = ensureValidDate(checkOutTime);
  
  if (!validCheckIn || !validCheckOut) {
    console.warn('Invalid dates in calculatePayableHours:', { checkInTime, checkOutTime });
    return 0;
  }
  
  // If shift type is null, try to determine it
  if (!shiftType) {
    shiftType = determineShiftType(validCheckIn);
  }
  
  try {
    console.log(`Calculating payable hours for ${format(validCheckIn, 'yyyy-MM-dd HH:mm')} to ${format(validCheckOut, 'yyyy-MM-dd HH:mm')}, shift: ${shiftType}, penalty: ${penaltyMinutes} minutes, isManualEdit: ${isManualEdit}`);
    
    // Handle night shift specially - use specific calculation
    if (shiftType === 'night') {
      return calculateNightShiftHours(validCheckIn, validCheckOut, penaltyMinutes, isManualEdit);
    }
    
    // Calculate minutes between check-in and check-out
    let diffInMinutes = differenceInMinutes(validCheckOut, validCheckIn);
    ...
    if (hours > 15.0) {
      ...
      if (shiftType === 'canteen') {
        ...
        const checkInHour = validCheckIn.getHours();
        if (checkInHour <= 7) {
          // 7AM shift
        ...
        earlyLeaveHour = 5;
        earlyLeaveMinute = 30;
      } else if (shiftType && SHIFT_TIMES[shiftType]) {
        // Regular shifts
        earlyLeaveHour = SHIFT_TIMES[shiftType].earlyLeaveTime.hour;
        earlyLeaveMinute = SHIFT_TIMES[shiftType].earlyLeaveTime.minute;
      } else {
        // Default early leave time if shift type is unknown
        earlyLeaveHour = 13; // 1 PM
        earlyLeaveMinute = 30; // 1:30 PM
      }
      
      // Check if checkout is after early leave time
      if (
        validCheckOut.getHours() > earlyLeaveHour || 
        (validCheckOut.getHours() === earlyLeaveHour && validCheckOut.getMinutes() >= earlyLeaveMinute)
      ) {
    ...
    console.error('Error calculating payable hours:', error);
    return 0;
  }
};
...
export const calculateNightShiftHours = (
  ...
  if (!checkInTime || !checkOutTime ||
      !(checkInTime instanceof Date) || !(checkOutTime instanceof Date) ||
      isNaN(checkInTime.getTime()) || isNaN(checkOutTime.getTime())) {
    console.warn('Invalid input to calculateNightShiftHours:', { checkInTime, checkOutTime });
    return 0;
  }
  ...
  try {
    ...
    console.error('Error calculating night shift hours:', error);
    return 0;
  }
};
...
export const isExcessiveOvertime = (checkOut: Date, shiftType: 'morning' | 'evening' | 'night' | 'canteen' | 'custom' | null): boolean => {
  ...
  if (!(checkOut instanceof Date) || isNaN(checkOut.getTime())) {
    console.warn('Invalid check-out time in isExcessiveOvertime:', checkOut);
    return false;
  }
  ...
  try {
    ...
    if (!SHIFT_TIMES[shiftType]) {
      console.warn('Unknown shift type in isExcessiveOvertime:', shiftType);
      return false;
    }
    ...
    console.error('Error checking for excessive overtime:', error);
  }
...
export const isLikelyNightShiftWorker = (records: TimeRecord[]): boolean => {
  if (!records || !Array.isArray(records) || records.length < 2) return false;
  ...
  try {
    ...
    const nightCheckIns = records.filter(record => {
      ...
      if (!record.timestamp || !(record.timestamp instanceof Date)) return false;
    ...
    console.error('Error checking for night shift worker:', error);
    return false;
  }
};
...
export const hasMorningShiftTimeRange = (timestamp: Date): boolean => {
  ...
  try {
    ...
    console.error('Error checking for morning shift time range:', error);
    return false;
  }
};
...
export const isMorningShiftPattern = (records: TimeRecord[]): number => {
  if (!records || !Array.isArray(records) || records.length < 2) return 0;
  ...
  try {
    ...
    for (const record of records) {
      if (!record.timestamp || !(record.timestamp instanceof Date)) continue;
    ...
    console.error('Error determining morning shift pattern:', error);
    return 0;
  }
};
...
export const isLikelyNightShiftCheckOut = (timestamp: Date): boolean => {
  ...
  try {
    const hour = safeGetHours(timestamp);
    return hour >= 5 && hour <= 7; // Early morning hours typical for night shift checkout
  } catch (error) {
    console.error('Error checking for night shift checkout:', error);
    return false;
  }
};
...
export const shouldHandleAsPossibleNightShift = (timestamp: Date): boolean => {
  ...
  try {
    ...
    console.error('Error checking for possible night shift:', error);
  }
...
export const isNightShiftCheckIn = (timestamp: Date): boolean => {
  ...
  try {
    ...
    console.error('Error checking for night shift check-in:', error);
    return false;
  }
};
...
export const isNightShiftCheckOut = (timestamp: Date): boolean => {
  ...
  try {
    ...
    console.error('Error checking for night shift check-out:', error);
    return false;
  }
};
...
export const isNightShiftPattern = (checkInTime: Date, checkOutTime: Date): boolean => {
  ...
  try {
    ...
    console.error('Error checking for night shift pattern:', error);
    return false;
  }
};
...
export const isLikelyFromNightShift = (record: any): boolean => {
  if (!record || !record.timestamp || !(record.timestamp instanceof Date) || isNaN(record.timestamp.getTime())) {
    return false;
  }
  ...
  try {
    ...
    console.error('Error checking if record is from night shift:', error);
  }
...
export const findNightShiftPair = (records: any[]): { checkIn: any | null, checkOut: any | null } => {
  ...
  try {
    ...
    if (nightCheckIn && nightCheckIn.timestamp instanceof Date) {
    ...
    console.error('Error finding night shift pair:', error);
    return { checkIn: null, checkOut: null };
  }
};
...
export const isEveningShiftPattern = (records: TimeRecord[]): boolean => {
  ...
  try {
    ...
    console.error('Error checking for evening shift pattern:', error);
    return false;
  }
};
...
export const hadEveningShiftOnDate = (records: TimeRecord[], dateStr: string): boolean => {
  if (!records || !Array.isArray(records) || !dateStr) return false;
  ...
  try {
    ...
    console.error('Error checking for evening shift on date:', error);
    return false;
  }
};
