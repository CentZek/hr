...
const ApprovedHoursPage: React.FC = () => {
      ...
      try {
        ...
        if (filterMonth === "all") {
          ...
          if (year && month) {
            ...
            // Default to current month if split fails
            startDate = format(startOfMonth(new Date()), 'yyyy-MM-dd');
            endDate = format(endOfMonth(new Date()), 'yyyy-MM-dd');
          }
        }
        ...
        setError('Failed to load double-time days. Please try refreshing the page.');
      }
    };
  ...
  useEffect(() => {
    const loadApprovedHours = async () => {
      ...
      try {
        ...
        setError('Failed to load approved hours data. Please try refreshing the page.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDeleteAllRecords = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        setError(`Failed to delete records: ${message}`);
      }
    } catch (error) {
      console.error('Error during deletion:', error);
      toast.dismiss(loadingToast);
      toast.error('An unexpected error occurred while deleting records');
      setError('An unexpected error occurred while deleting records. Please try again.');
    } finally {
      setIsDeleting(false);
  ...
  const handleHolidaysUpdated = async () => {
    ...
    try {
      ...
      setError('Failed to refresh data after calendar update. Please try again.');
    }
  };
  ...
  // If there's an error, display it at the top of the page
  if (error) {
    return (
            ...
            <div className="bg-red-50 border border-red-200 rounded-md p-4 mb-4">
              ...
              <div className="mt-4">
                <button
                  ...
                  className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-red-600 hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                >
                  Refresh Page
                </button>
                <button
